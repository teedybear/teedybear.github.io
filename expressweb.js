...
app.get('/testCMD', function (req, res) {            //讀區塊練的部分
...
app.get('/login', function (req, res) {
  ...
  if (!acct || !pwd) {
    ...
    conn.connect().then(function () {
      var sqlQuery = "select * from member where memberno='" + acct + "'"; //先讀帳號
      ...
      reqsql.query(sqlQuery).then(function (result) {
        ...
        if (result.recordset.length == 0) {
          ...
          sqlQuery = "select * from member where memberno='" + acct + "' and pwd='" + pwd + "'";//讀帳號密碼
...
app.get('/activityMaintain', function (req, res) {              //活動維護
  validSession(req, res);
  conn.connect().then(function () {
    var sqlQuery = "select * from activity where activitydate > GETDATE()";//大於今天日期
    var reqsql = new sql.Request(conn);
    reqsql.query(sqlQuery).then(function (result) {
      //console.log(result.recordset);
      //console.log(result.recordset.length);
...
app.get('/activityMaintainDetail/:no', function (req, res) {  //活動維護細節

  console.log(req.session.status);
  validSession(req, res);

  var no = req.params.no; //取:的no
  //var id = req.query.id;
  ...
  if (type == 'delete') {//刪除
    ...
    conn.connect().then(function (err) {
      const ps = new sql.PreparedStatement(conn)  //PreparedStatement
      ps.input('id', sql.Int)
...
/** 參加活動 Start */
app.get('/joinActivity', function (req, res) {   //選擇全部活動where活動人in(選擇活動人在參加活動裡 where memberno =session memberno 和 thisstatus = 'thisstatus
  validSession(req, res);                              //抓到這帳號的參加活動
  conn.connect().then(function () {
    ...
    var request = new sql.Request(conn);
    var his_activity = null;                         //1是參加活動  0是沒參加
...
app.get('/joinActivityDetail', function (req, res) {
  validSession(req, res);
  var activityno = req.query.activityno;    //抓參加活動編號
  conn.connect().then(function () {                 //選擇activity   WHERE  activityno= 抓到的 activityno
...
app.get('/inventory', function (req, res) {
  ...
  conn.connect().then(function () {
    ...
    var request = new sql.Request(conn);  //物品清單
    request.query(sqlQuery).then(function (result) {
    ...
    request.query(sqlQuery).then(result => {
      ...
      if (result.recordset.length > 0) {
        result.recordset.forEach(element => {
          console.log("forEach Start")
          var transaction = new sql.Transaction(conn);
          transaction.begin(err => {   //transaction
            const request = new sql.Request(transaction)
            //   let allactivityid = "'" + getactivityid.join("','") + "'"
            let sqlUpdate = "update member set point=point-'" + point + "' where  memberno='" + mymemberno + "'and pwd='" + mypwd + "'"
